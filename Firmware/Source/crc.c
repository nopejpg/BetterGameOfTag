...
//Lookup table built using TI slaa221.zip
...
/******************************************************************************\
* FUNCTION			Util_crc16
*	DESCRIPTION		Calculates the raw crc using the CCITT polynomial lookup table
...
{
  while(length--)
    crc = crcLookUpTable[(crc ^ *pData++) & 0xff] ^ (crc >> 8);

  return crc;
}

/******************************************************************************\
* FUNCTION			CRC_slowCRC16
*	DESCRIPTION		Calculates the raw crc using the CCITT polynomial
* PARAMETERS		pData - location of data
*               length - length of data
...
uint16_t CRC_slowCRC16(uint8_t *pData, uint32_t length, uint16_t crc)
{
  ...
  uint8_t aByte;
  uint8_t temp;
  const uint8_t *pTempData = (const uint8_t*) pData;

  do
  {
    for(i = 0, aByte = *pTempData++; i < 8; i++, aByte >>= 1)
    {
      temp = crc & 0x01;
      crc >>= 1;
      if(temp ^ (aByte & 0x01))
